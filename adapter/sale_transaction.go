...
func (etl SrToClearanceETL) Load(ctx context.Context, source interface{}) error {
	...
	for _, saleTransaction := range saleTransactions {
		_, dbSaleTransactions, err := models.SaleTransaction{}.GetSaleTransactions(ctx, models.RequestInput{TransactionId: saleTransaction.TransactionId,
			ShopCode: saleTransaction.ShopCode, TransactionChannelType: saleTransaction.TransactionChannelType, MaxResultCount: 1})
		if err != nil {
			return err
		}
